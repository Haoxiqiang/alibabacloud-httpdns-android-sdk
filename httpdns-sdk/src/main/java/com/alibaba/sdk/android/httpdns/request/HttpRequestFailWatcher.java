    ...
    public void onFail(HttpRequestConfig config, Throwable throwable) {
        if (reportManager != null) {
            if (config.url().contains("/ss")) {
                reportManager.reportErrorSCRequest(config.getIp(), ReportUtils.getErrorCode(throwable) + "", ReportUtils.getErrorMsg(throwable), 0);
            } else {
                reportManager.reportErrorHttpDnsRequest(config.getIp(), ReportUtils.getErrorCode(throwable) + "", ReportUtils.getErrorMsg(throwable), 0, 0);
            }
        }
    }
}
